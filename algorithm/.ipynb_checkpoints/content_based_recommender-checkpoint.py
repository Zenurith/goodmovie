...
class ContentBasedRecommender:
    ...
    def fuzzy_search_movie(self, title: str, threshold: int = None) -> Tuple[str, int, int]:
        """
        ...
        # Use fuzzy matching
        matches = process.extract(title, self.movie_mappings['titles'], limit=5)
        
        if matches and matches[0][1] >= threshold:
            matched_title, score = matches[0]
            idx = self.movie_mappings['titles'].index(matched_title)
            self.logger.info(f"Fuzzy match: '{title}' -> '{matched_title}' (score: {score})")
            return matched_title, score, idx
        
        # Show available matches
        if matches:
            suggestions = [f"{match[0]} ({match[1]}%)" for match in matches[:3]]
